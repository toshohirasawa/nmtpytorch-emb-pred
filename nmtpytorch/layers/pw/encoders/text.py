...
class TextEncoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, n_vocab, rnn_type,
                 ...
                 emb_init_type='random', emb_init_args={}):
        super().__init__()
        ...
        self.emb_init_type = emb_init_type.lower()
        self.emb_init_args = emb_init_args

        assert self.emb_init_type in ('random', 'fasttext'), \
            f'Unknown emb_init_type parameter: {self.emb_init_type}'
        ...
        self.emb = getattr(self, f'_emb_init_{self.emb_init_type}')()
    ...
    def _emb_init_random(self):
        return nn.Embedding(self.n_vocab, self.input_size,
            padding_idx=0, max_norm=self.emb_maxnorm,
            scale_grad_by_freq=self.emb_gradscale)
    
    def _emb_init_fasttext(self):
        for kw in ('pretrained_file', 'vocab', 'sub_center', 'sub_pca', 'n_pca'):
            assert kw in self.emb_init_args, \
                f"f'{kw}' should be given in 'emb_init_args' section"
        vocab = self.emb_init_args['vocab']
        fname = self.emb_init_args['pretrained_file']
        # load weight from fasttext and move it to GPU
        w = load_fasttext(vocab, fname, normalize=True,
            sub_center=self.emb_init_args['sub_center'],
            sub_pca=self.emb_init_args['sub_pca'],
            pca_size=self.emb_init_args['n_pca']).cuda()
        return nn.Embedding(self.n_vocab, self.input_size,
            padding_idx=0, max_norm=self.emb_maxnorm,
            scale_grad_by_freq=self.emb_gradscale,
            _weight=w)
